...
guarda en Cloudflare KV. El corrector depende del tipo: `practice` y
`formula` usan Mistral (como siempre); `exam` usa Fable 5 primero
(mismo modelo que arma el borrador, con `tool_choice` forzado y un
prompt que pide re-derivar cada función en vez de solo revisar que
"suene" bien) y cae a Mistral solo si Fable falla o no valida -- ver
...
| `ANTHROPIC_API_KEY` | IA que crea el borrador (Claude/Fable), y que corrige `exam` (Fable) |
| `MISTRAL_API_KEY` | IA que corrige `practice`/`formula` (única), y fallback de corrección para `exam` si Fable falla |
...
- Qué IA arma o corrige el contenido (Claude/Fable/Mistral) es un
  ...
  Por eso el cambio Fable/Mistral para `exam` (arriba) no requiere
  tocar nada en el Worker — la key y el schema (`{modelos: [...]}`)
  quedan idénticos.
