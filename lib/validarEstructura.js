...
// Límites de largo para cada campo de "necesitaHerramienta" (ver más abajo
// y armarToolClaude("visual") en prompts.js) -- generosos para que la IA
// pueda explicarse bien, pero acotados para que no se vuelva un ensayo
// (esto lo va a leer una persona en visual_later, no hace falta más).
const LARGO_MIN_CAMPO_NECESITA_HERRAMIENTA = 10;
const LARGO_MAX_CAMPO_NECESITA_HERRAMIENTA = 600;
...
/**
 * Valida el campo "necesitaHerramienta" -- camino alternativo a
 * "formulas" (ver armarToolClaude("visual") en prompts.js): la IA lo usa
 * en vez de forzar una fórmula que no representa bien el tema, cuando ni
 * los 3 formatos de fórmula ni ninguno de los comandos que YA tiene
 * visual.js (Circle, tangent, area, riemann, taylor, field, matrix,
 * Surface, etc. -- ver CAPACIDADES_VISUAL_JS_COMPLETAS en prompts.js)
 * alcanzan para este tema. No valida contenido matemático (acá no hay
 * fórmula que compilar) -- solo que los 3 campos vengan como texto
 * explicativo real, ni vacíos ni un ensayo.
 */
function validarNecesitaHerramientaVisual(nh) {
  const errores = [];
  if (typeof nh !== "object" || nh === null) {
    return { ok: false, errores: ['"necesitaHerramienta" tiene que ser un objeto'] };
  }
  for (const campo of ["tema", "motivo", "herramienta_sugerida"]) {
    const valor = nh[campo];
    if (typeof valor !== "string" || valor.trim().length < LARGO_MIN_CAMPO_NECESITA_HERRAMIENTA) {
      error(errores, `necesitaHerramienta.${campo}: falta o es demasiado corto (mínimo ${LARGO_MIN_CAMPO_NECESITA_HERRAMIENTA} caracteres)`);
    } else if (valor.length > LARGO_MAX_CAMPO_NECESITA_HERRAMIENTA) {
      error(errores, `necesitaHerramienta.${campo}: demasiado largo (máximo ${LARGO_MAX_CAMPO_NECESITA_HERRAMIENTA} caracteres)`);
    }
  }
  return { ok: errores.length === 0, errores: errores.length > 0 ? errores : null };
}
...
export function validarVisual(data) {
  ...
  // Camino "no cobertura" -- ver validarNecesitaHerramientaVisual arriba.
  // Se chequea ANTES que "formulas": si la IA mandó esto, no tiene sentido
  // exigirle también una lista de fórmulas (son alternativas, no las dos
  // a la vez -- ver el "anyOf" del schema en armarToolClaude("visual")).
  if (data.necesitaHerramienta !== undefined) {
    return validarNecesitaHerramientaVisual(data.necesitaHerramienta);
  }
