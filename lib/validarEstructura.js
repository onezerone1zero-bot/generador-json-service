...
/**
 * Valida visual.json: { formula: "string, expresión evaluable f(x,y)" }
 *
...
export function validarVisual(data) {
  ...
  if (typeof data.formula !== "string" || data.formula.trim() === "") {
    return { ok: false, errores: ['falta "formula" o no es string'] };
  }

  try {
    compilarFormula(data.formula);
  } catch (err) {
    error(errores, `"formula" no compila con la gramática del frontend: ${err.message}`);
  }
