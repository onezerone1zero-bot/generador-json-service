...
//   CLOUDFLARE_NAMESPACE_ID_VISUAL     (namespace ID de visual_default)
//   CLOUDFLARE_NAMESPACE_ID_VISUAL_LATER (namespace ID de visual_later)
...
// formula_JSON, practice_JSON y visual_default son namespaces DISTINTOS
// (cada uno con su propio ID en Cloudflare), no un solo namespace con
// prefijos de key. Si CLOUDFLARE_NAMESPACE_ID_EXAM no está seteada, cae a
// PRACTICE por retrocompatibilidad -- pero FORMULA y VISUAL son
// obligatorias si se piden esos tipos (no tienen fallback: escribir
// fórmulas de práctica/examen en el namespace equivocado por un fallback
// silencioso sería peor que fallar fuerte).
const NAMESPACE_ID_POR_TIPO = {
  ...
  visual: process.env.CLOUDFLARE_NAMESPACE_ID_VISUAL,
  // Namespace nuevo, separado de "visual" (visual_default): acá se anota
  // CUANDO la IA de tipo "visual" declara que no puede armar una fórmula
  // default decente para un tema (ver validarNecesitaHerramientaVisual en
  // lib/validarEstructura.js y la rama correspondiente en generarUnTipo,
  // lib/generar.js) -- qué tipo de herramienta haría falta (función nueva
  // en visual.js, o una herramienta externa aparte), para poder construirla
  // más adelante en vez de perder ese caso silenciosamente.
  visual_later: process.env.CLOUDFLARE_NAMESPACE_ID_VISUAL_LATER,
};
 ...
 */
export async function escribirJSON(key, valor, tipo, { ttlSegundos, idioma = "es" } = {}) {
  ...
  // que no se manda acá (no se parsea de la key). El idioma llega por
  // parámetro (ya no sale de process.env.IDIOMA -- esta instancia ya no
  // tiene un idioma fijo propio, ver server.js/generar.js).
  if (!ttlSegundos) {
...
// server.js). Todo lo que llame a keyPractice/keyExam/keyFormulas tiene
// que pasar el idioma explícito.
 ...
 * @param {string} [idioma] - idioma destino, default "es"
 */
export function keyPractice(slugMateria, slugTema, slugTemaCanonico, idioma) {
  return `practice:${slugMateria}--${slugTemaCanonico || slugTema}${sufijoIdioma(idioma)}`;
}

/** Key de KV para el exam.json de un tema. Prefijo "exam:". Ver keyPractice. */
export function keyExam(slugMateria, slugTema, slugTemaCanonico, idioma) {
  return `exam:${slugMateria}--${slugTemaCanonico || slugTema}${sufijoIdioma(idioma)}`;
}

/** Key de KV para el formulas.json de un tema. Prefijo "formulas:". Ver keyPractice. */
export function keyFormulas(slugMateria, slugTema, slugTemaCanonico, idioma) {
  return `formulas:${slugMateria}--${slugTemaCanonico || slugTema}${sufijoIdioma(idioma)}`;
}

/**
 * Key de KV para el visual.json de un tema (fórmula default pre-cargada
 * en el campo "Formula" de visual.html). Prefijo "visual:". Ver
 * keyPractice. Namespace propio: visual_default (ver
 * CLOUDFLARE_NAMESPACE_ID_VISUAL arriba) -- distinto de subject_JSON y
 * de los otros tres namespaces de este service.
 */
export function keyVisual(slugMateria, slugTema, slugTemaCanonico, idioma) {
  return `visual:${slugMateria}--${slugTemaCanonico || slugTema}${sufijoIdioma(idioma)}`;
}

/**
 * Key de KV para la anotación de "herramienta faltante" de un tema en el
 * graficador (namespace propio: visual_later, ver NAMESPACE_ID_POR_TIPO
 * arriba). Prefijo "visual_later:". Mismo criterio materia+tema+idioma
 * que keyVisual -- MISMA identidad, namespace distinto, así que no hace
 * falta que la key en sí lleve nada más para no colisionar con keyVisual.
 */
export function keyVisualLater(slugMateria, slugTema, slugTemaCanonico, idioma) {
  return `visual_later:${slugMateria}--${slugTemaCanonico || slugTema}${sufijoIdioma(idioma)}`;
}
